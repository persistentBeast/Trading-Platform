...
func handleWebSocketMessage(client *Client, message []byte) {
	...
	case "subscribe":
		...
		client.symbols = make([]string, len(symbols))
		for i, sym := range symbols {
			client.symbols[i] = sym.(string)
		}
...
func handleNATSMessage(data []byte) {
	...
	stockTick := StockTick{
		Symbol:    symbol,
		Price:     price,
		Timestamp: unixNanoInt, // Store the timestamp as int64
	}

	// Send the price to all connected WebSocket clients subscribed to that symbol concurrently
	clientsMutex.Lock()
	for client := range clients {
		if contains(client.symbols, symbol) {
			go sendPriceToClient(client, stockTick)
		}
	}
	clientsMutex.Unlock()
}

func sendPriceToClient(client *Client, tick StockTick) {
